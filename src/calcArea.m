function Area = calcArea(xm,ym,xbi,xbe,ybi,ybe,n)

    N = 1000; % Inner Resolution
    
    Nx = N;
    Ny = N;
    xstart = min([xbi;xbe]) - 5e-4;
    ystart = min([ybi;ybe]) - 5e-4;
    xend = max([xbi;xbe]) - 5e-4;
    yend = max([ybi;ybe]) - 5e-4;
    ...
    for i = 1:Nx
        for j = 1:Ny
            ...
            for k = 1:n
                if ((xp-xm(k))^2 + (yp-ym(k))^2) < (0.1*h)^2
                    far = false;
                    break
                end
            end
